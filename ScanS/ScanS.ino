...
{
	Serial.print("Model: ");
	Serial.print("Arduino MEGA, smal");
	Serial.print("ver: ");
	Serial.println("4.00");
}
...
void Pos(byte p=0)
{
	if(!p)
	{
		for (byte j=0; j<=2; j++)
		{
			long int g=eng[j].pos;
			Serial.print(g);
			Serial.print(",");
		}
		long int g=eng[3].pos;
		Serial.println(g);
	}
	else {long int g=eng[p-1].pos; Serial.println(g);}
}
...
void timerIsr()
{
	...
	for(volatile byte engNum=0; engNum<=3; engNum++) // пройтись по всем двигателям
	{
		...
		{    //вперед
			if(eng[engNum].pos>destination[engNum])  eng[engNum].SetStat(3);  //если дальше нужного положения, то останавливаем		
		}
		else 
		{    //назад
			if(eng[engNum].pos<destination[engNum])  eng[engNum].SetStat(3);	//если ближе нужного положения, то останавливаем 
		}
