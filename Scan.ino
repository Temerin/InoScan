...
int pinEncoder1[4]={1,2,34,44}; //распиновка условная. Нужно менять при сборке. Каждый двигатель должен соответствовать своему энкодеру.
int pinEncoder2[4]={1,2,35,45};
...
{	
	...
	EncoderH(int pin1, int pin2, byte num)
	{
		...
		pinMode(encPin[0], INPUT); 
		pinMode(encPin[1], INPUT);
    prevLevel[0] = digitalRead(encPin[0]);     // считываем состояние 1 выхода энкодера 
    prevLevel[1] = digitalRead(encPin[1]);     // считываем состояние 2 выхода энкодера  
		//oldPos = sOldPos;
	}
  // Текущее состояние
  ...
  void Update()
  {
    level[0] = digitalRead(encPin[0]);     // считываем состояние 1 выхода энкодера 
    level[1] = digitalRead(encPin[1]);     // считываем состояние 2 выхода энкодера  
...
class Engine
{
  ...
  unsigned long currentMillis; // текущее время в миллисекундах
	...
	{
		if(forDir) digitalWrite(dirPin, LOW); 
		else digitalWrite(dirPin, HIGH); 
		currentMillis = millis(); // текущее время в миллисекундах
		if((engState == HIGH) && (currentMillis - previousMillis >= m))
		{
		  //Serial.println("1 ");
		  engState = LOW; // выключаем
		  previousMillis = currentMillis; // запоминаем момент времени
		  digitalWrite(stepPin, LOW);; // реализуем новое состояние
		}
		else if ((engState == LOW) && (currentMillis - previousMillis >= s))
		{
		  //Serial.println("2");
...
void setup() 
{
	...
	for(byte i=0; i<4; i++)
	{
		pinMode(pinOpen[i], INPUT);
	}
	//------ Timer1 ----------
	TCCR1B = (1<<WGM12);    // Режим CTC (сброс по совпадению)
	TCCR1B |= (1<<CS11);    // Тактирование от CLK.
	...
	OCR1A = 2000;           // Верхняя граница счета. Диапазон от 0 до 65535.
...
ISR (TIMER1_COMPA_vect)
{
	...
	for(volatile byte engNum=0; engNum<=3; engNum++) // пройтись по всем двигателям
	{
		...
		{
      if(digitalRead(pinOpen[engNum])) //при нахождении остановится и обнулить позицию
      {
